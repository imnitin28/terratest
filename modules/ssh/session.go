...
/* SshSession is a container object for all resources created by an SSH session. The reason we need this is so that we can do a single defer
 in a top-level method that calls the Cleanup method to go through and ensure all of these resources are released and cleaned up.*/
...
func (sshSession *SshSession) Cleanup(t testing.TestingT) {
	...
	/* Closing the session may result in an EOF error if it's already closed (e.g. due to hitting CTRL + D), so don't report those errors, 
	as there is nothing actually wrong in that case. */
...
func (jumpHost *JumpHostSession) Cleanup(t testing.TestingT) {
	...
	/* Closing a connection may result in an EOF error if it's already closed (e.g. due to hitting CTRL + D), so don't report those errors, 
	as there is nothing actually wrong in that case. */
...
/* Go is a shitty language. Checking an interface directly against nil does not work, and if you don't know the exact types the interface may be 
