...
stderr of that command will also be logged with Command.Log to make debugging easier. If there are any errors, fail the test.*/
...
stdout and stderr of that command will also be printed to the stdout and stderr of this Go program to make debugging easier.*/
...
/*This function captures stdout and stderr into the given variables while still printing it to the stdout and stderr of this Go program */
...
func readData(t testing.TestingT, log *logger.Logger, reader *bufio.Reader, writer io.StringWriter) error {
	...
	for {
		...
		/*only return early if the line does not have any contents. We could have a line that does not not have a newline before io.EOF, we still
		need to add it to the output.*/
		if len(line) == 0 && readErr == io.EOF {
			break
		}

		/*logger.Logger has a Logf method, but not a Log method. We have to use the format string indirection to avoid interpreting any possible formatting characters in
		the line. See https://github.com/gruntwork-io/terratest/issues/982.*/
		log.Logf(t, "%s", line)
...
func GetExitCodeForRunCommandError(err error) (int, error) {
	...
	if exitErr, ok := err.(*exec.ExitError); ok {
		...
		This works on both Unix and Windows. Although package syscall is generally platform dependent, WaitStatus is defined for both Unix and Windows 
		and in both cases has an ExitStatus() method with the same signature.*/
